...
public partial class LearningAppDbContext : DbContext
{
    public LearningAppDbContext()
    {
    }

    public LearningAppDbContext(DbContextOptions<LearningAppDbContext> options)
        : base(options)
    {
    }
    ...
    {
        // Only configure if it's not already been configured by DI (e.g., AddDbContextPool)
        if (!optionsBuilder.IsConfigured)
        {
            optionsBuilder.UseSqlServer("Server=(localdb)\\MSSQLLocalDB;Initial Catalog=LearningAppDB;Integrated Security=SSPI;MultipleActiveResultSets=true;TrustServerCertificate=True;");
        }
    }
